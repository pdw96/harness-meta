...
## 9-stage workflow (v2.0+) + two-track operation (v6.2+/v8.1+)
...
| B. INTENT | `.../MILESTONE.md` 안 `## INTENT` | 의도 — goal / motivation / success_criteria / out_of_scope / dependencies |
| C. RESEARCH | `.../MILESTONE.md` 안 `## RESEARCH` | 조사 — external / codebase / options / risks_identified |
| D. DESIGN | `.../MILESTONE.md` 안 `## DESIGN` | 설계 — decisions / approach / phases / risk_mitigation + N+ 관점 검토 |
| E. APPROVE | `.../MILESTONE.md` 안 `## APPROVE` | 사용자 명시 승인 게이트 — approved_by / date / approval_summary |
| F. EXECUTE | `.../execute/phase-{n}.md` | per-phase 구현 (1 phase = 1 commit, conventional commits) |
| G. VERIFY | `.../MILESTONE.md` 안 `## VERIFY` | 검증 — smoke / criteria_check vs INTENT / verdict |
| H. REPORT | `.../MILESTONE.md` 안 `## REPORT` | 종합 backward — summary / delta / lessons_learned |
| I. PROPOSE | `.../MILESTONE.md` 안 `## PROPOSE` | 후속 forward — next_candidates ROADMAP 등록 + archival trigger |

## 대상 구분

| 대상 | 경로 | 진입 조건 |
|------|------|---------|
| **메타 milestone** | 큰 건: `~/harness-meta/development/milestones/v{X.Y}/MILESTONE.md` / 작은 건: `.../LIGHTWEIGHT.md` | argument 부재 또는 `meta` 또는 CWD=harness-meta |
| **프로젝트별 하네스 개선** | 큰 건: `<project-repo>/milestones/v{X.Y}/MILESTONE.md` / 작은 건: `.../LIGHTWEIGHT.md` | argument=`<name>` + `~/harness-meta/projects/<name>/` 존재 + `.harness.toml` 존재 |
...
6. ROADMAP `milestones[]` 배열에 신규 항목 추가 (in_progress entry) — v6.2+ 신 schema (`{version: "v{X.Y}", id: "{group-slug}", title, status: "in_progress", summary, trigger, milestones_path: "milestones/v{X.Y}/MILESTONE.md#sub-milestones"}`). v3.0~v6.1 bundled 보존 entry 는 `milestones_path: "milestones/v{X.Y}/milestones.md"` 그대로 둔다. **v5.21+ schema A2**: `milestones[]` = recent 3 completed + in_progress + deferred only. PROPOSE 발의 후보는 `next_candidates[]` 별도 필드 (Stage I 안 등재). 과거 completed entry archival = **GitHub Releases** (v6.19+, v8.13 두 트랙 정합), v6.19 이전 history 는 `CHANGELOG.md` historical hybrid 로 보존.
...
7. **`milestones/v{X.Y}/MILESTONE.md` 안 `## SUB_MILESTONES` 섹션 스켈레톤 즉시 작성** (v6.2+ 9-stage-flattened era 의무, narrative 1차 source — v3.0~v6.1 9-stage-bundled era 의 별도 `milestones.md` 파일 흡수). step 6 의 ROADMAP entry `milestones_path` 와 1:1 매핑 강제 (anchor `#sub-milestones`). skeleton 최소 필드 (## SUB_MILESTONES 섹션 안 ```json``` 블록):
   ...
   **placeholder title 허용 narrative**: OPEN 시점에서는 정확한 phase 분할 미확정 — phase-1 title placeholder 허용, Stage D DESIGN 단계에서 phases[] 확정 후 `MILESTONE.md ## SUB_MILESTONES` 를 1:1 동기 갱신 (placeholder title 교체).

   **v6.2+ mitigation**: `MILESTONE.md` 부재 시 `tests/_era_detect.py` 가 9-stage-flattened 표지를 찾지 못해 신규 milestone 을 오분류한다. 또한 `tests/smoke-bundle-trigger.sh` 가 `status: in_progress` entry 의 `milestones_path` 필드 + 실 파일 존재를 검증한다 (status: pending → continue / in_progress|completed → 검증). step 7 가 본 검증 분기와 narrative 부합 — OPEN 단계 종료 시점에 ROADMAP entry status: in_progress + milestones_path 보유 + 실 파일 보유 = 3 조건 동시 충족.

### Stage B — INTENT (의도)

```
milestones/v{X.Y}/MILESTONE.md ## INTENT
```
...
⚠️ phase list / file list / commit 메시지 등 implementation detail은 **## DESIGN 으로 미룸**.
...
```
milestones/v{X.Y}/MILESTONE.md ## RESEARCH
```
...
### Stage D — DESIGN (설계 + N+ 관점 검토)

```
milestones/v{X.Y}/MILESTONE.md ## DESIGN
```
...
**decisions / phases 부산물 정책** (v3.10): `decisions[i].rationale` 와 `phases[n].scope` 는 (a) 본 milestone 의 결정 / 단계 범위 **사실 진술**만 — `## PROPOSE` `next_candidates` 발의 narrative 같이 forward propose 책임 직접 거명 **금지**. (b) 본 milestone 안 결정 / 범위 자체가 후속 candidate source 가 될 수 있으나, 명명 + ROADMAP 등재는 Stage I (PROPOSE) 통합 흡수.
...
4. **결과 흡수** — scope 안 결과 → `## DESIGN` / scope 외 거명 → MILESTONE.md `## SCOPE_OUT_NOTES` (조건부 H2)

**scope 안/외 분리**: scope 안 = INTENT.success_criteria + DESIGN.phases 직접 정합 본질 (`## DESIGN` 흡수). scope 외 = scope 본질 외 거명 (관련 본질 / 후속 candidate / 우연 발견) → `## SCOPE_OUT_NOTES` 거주만, **next_candidates 자동 append 부재** (PROPOSE stage 안 사용자 명시 결정 게이트 후만 등재 — 부산물 cycle 차단).

**의견 충돌 처리**: 충돌 발견 시 `AskUserQuestion` 자동 invoke (각 충돌 1 question, 최대 4 question).

**Claude Code × Codex cross-check (선택 보조 트랙)**:

컨설팅 자산 영향이 있거나 운영 문서 drift 위험이 큰 변경은 DESIGN 완료 직후 또는 VERIFY 직전에 Codex 교차 검토를 권장한다. 책임 분리:

- Claude Code = 작성자 / 운영자 — milestone narrative 작성, Claude Code plugin·agent·skill spec 적용, smoke 실행.
- Codex = 독립 감사자 — current/historical era 혼용, README/AGENTS/CLAUDE/command drift, plugin manifest 설명과 실제 파일 inventory 불일치, smoke 가 못 잡는 운영 지시 리스크 점검.

산출물 흡수 위치:

- 큰 건: `MILESTONE.md ## DESIGN` 또는 `## VERIFY` 안 `codex_cross_check` 단락/JSON block.
- 작은 건: `LIGHTWEIGHT.md ## 기록` 안 `Codex cross-check` 단락.
- 외부 audit: `projects/<name>/audit-YYYY-MM-DD/codex-cross-check.md` 선택 생성.

자동 강제는 하지 않는다. 첫 도입 본질은 **권장 검토 트랙**이며, smoke/hook 강제는 별도 사용자 명시 결정 후 신규 milestone 으로 발의한다.

> **적용 범위** (breaking change 최소): v7.0+ 신규 milestone 만 본 mechanism. 기존 28 active milestone DESIGN 의 5 관점 narrative 는 historical 로 자연 보존 (디스크 보존). 본 N+ 가변 mechanism 첫 실사용 = v7.1 (v7.0 = 설치만).

**Stage D 완료 직전 의무 step** (v3.5_open-stage-discipline-strengthening phase-2 도입):

`phases[]` 확정 직후 (N+ 관점 검토와 필요한 Codex cross-check 의견 흡수 후) → `milestones/v{X.Y}/MILESTONE.md` 안 `## SUB_MILESTONES` JSON block 을 `phases[]` 와 1:1 동기 갱신 (placeholder title 교체). Stage A step 7 의 forward cross-ref 와 backward cross-ref → 양방향. 본 step 미실행 시 `## SUB_MILESTONES` 가 OPEN 단계 placeholder 잔존 (stale narrative) → smoke-spec-verification 산출물 검증 시 narrative drift 침묵 통과 위험.

### Stage E — APPROVE (사용자 명시 승인 게이트)

```
milestones/v{X.Y}/MILESTONE.md ## APPROVE
```
...
- `approval_summary` (N+ 관점 검토 결과 + DESIGN 종합 narrative)

**필수 게이트**: N+ 관점 검토 결과 + DESIGN 종합 → `AskUserQuestion`으로 사용자 명시 승인 받음 → `## APPROVE` 작성 (`approval.approved_by: "user"` + `date` ISO-8601). **미승인 시 EXECUTE 진입 금지**.
...
**선결 조건 (v6.2+ 9-stage-flattened era, EXECUTE 진입 전 의무)**

- `milestones/v{X.Y}/MILESTONE.md` 보유 확인 — 이미 **Stage A step 7 에서 작성됨**. EXECUTE 진입 직전 확인만 — 보조 검증 step (예: `test -f milestones/v{X.Y}/MILESTONE.md`). 부재 시 OPEN 단계 누락 = step 7 retroactive 작성 후 진행. Stage D DESIGN 단계 phases[] 확정 후 `## SUB_MILESTONES` 1:1 동기 갱신 의무.
- **INTENT~APPROVE commit 시점** — 3 패턴 중 선택 (v3.1 L6):
  ...
  기본값 (b): Stage G commit 에 MILESTONE.md 의 INTENT/RESEARCH/DESIGN/APPROVE 섹션 포함 → 산출물 소실 없음.
...
```
milestones/v{X.Y}/MILESTONE.md ## VERIFY
```
...
### Stage H — REPORT (종합 backward)

```
milestones/v{X.Y}/MILESTONE.md ## REPORT
```
...
⚠️ `next_candidates` (forward) 는 **## PROPOSE 로 분리** — REPORT 는 backward 종합만.

### Stage I — PROPOSE (후속 forward + ROADMAP 등록)

```
milestones/v{X.Y}/MILESTONE.md ## PROPOSE
```
...
3. **Archival cycle** (v6.19+ / v8.13 정합): `milestones[]` 안 completed entry count > 3 인 경우 가장 오래된 completed entry 를 ROADMAP 에서 trim 하고, 영구 보존은 GitHub Release 로 위임한다. 9-stage 는 `MILESTONE.md ## REPORT`, 가벼운 흐름은 `LIGHTWEIGHT.md ## 기록` 이 release body source 다. commit marker `[release:v{X.Y}]` 또는 workflow_dispatch 가 release trigger. v6.19 이전 history 는 `CHANGELOG.md` historical hybrid 로 보존한다.
...
4. **PR 생성 + main 머지** (사용자 결정):
   ...
   ```bash
   gh pr create --title "milestone v{X.Y}" --body "..."
   ```
