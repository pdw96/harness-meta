...
| 완료 세션 | 진행 일자 | 산출 |
|---------|---------|------|
| **v1.88_precommit-performance** (milestone) | 2026-05-07 | pre-commit 4 local hook `always_run: true` 제거 + 의미론적 `files:` 패턴 추가. `.pre-commit-config.yaml` (smoke-spec-verification: `sessions/.*\.md$\|milestones/.*\.md$` / smoke-scope-contract: `sessions/.*\.md$\|bootstrap/docs/OWNERSHIP\.md$\|claude/commands/harness-meta\.md$` / smoke-cross-ref: `\.md$` / smoke-claude-md-drift: `CLAUDE\.md$\|tests/smoke-.*\.sh$`) + `tests/CLAUDE.md` 현황표 갱신 (v1.80→v1.88). 효과 실측: phase-2 commit (`tests/CLAUDE.md` 단일 staged) 시 spec-verification + scope-contract 모두 "no files to check Skipped" 출력 — 비관련 hook 자동 skip 검증. 5 pre-commit cycle 40분 wall-clock evidence (v1.87 milestone) 해소 목표 — shell/py/yaml 커밋 시 4 smoke 완전 skip. context7 `pre-commit` `always_run`/`files`/`pass_filenames` 필드 의미론 v1.x 일관 drift=no. 회귀 0 (spec-verification 608/SKIP=4 + scope-contract 198/198 + cross-ref 1/1 + claude-md-drift 16/16 + roadmap-sync 31/0). architecture review (3 관점) — 초안 `smoke-scope-contract: sessions/.*\.md$` false negative (OWNERSHIP/harness-meta.md 단독 변경 skip) 발견 → 패턴 3분기 보정. L1: files: 가드 즉각 효과 — phase별 staging set로 4 hook 모두 SKIP 사례 누적. L2: milestone PLAN.md MD032 수동 빈 줄 삽입 의무 (markdownlint --fix 외부). L3: architecture review 가치 (false negative 차단). L4: SKILL.md acceptable omission (수정 빈도 낮음). 후속 §3-B `v1.88b-precommit-pass-filenames-incremental` (L2 효과 측정 후) + `v1.88c-markdownlint-autofix` (PLAN.md MD032 evidence 1→3+). 상세 → `milestones/v1.88_precommit-performance/REPORT.md`. |
...
- **v1.88_precommit-performance** (2026-05-07) — pre-commit 4 local hook `always_run: true` 제거 + 의미론적 `files:` 패턴 추가. 변경 2 파일 (`.pre-commit-config.yaml` 4 hook entry + `tests/CLAUDE.md` 현황표 갱신). 효과 실측: phase-2 commit (`tests/CLAUDE.md` 단일 staged) 시 spec-verification + scope-contract 모두 "no files to check Skipped" — 비관련 hook 자동 skip 검증. 5 pre-commit cycle 40분 wall-clock evidence (v1.87 milestone) 해소 목표. context7 `pre-commit` `files`/`always_run`/`pass_filenames` 의미론 v1.x 일관 drift=no. 회귀 0 (spec-verification 608/SKIP=4 + scope-contract 198/198 + cross-ref 1/1 + claude-md-drift 16/16 + roadmap-sync 31/0). architecture review 3 관점에서 초안 `smoke-scope-contract: sessions/.*\.md$` false negative (OWNERSHIP/harness-meta.md 단독 변경 skip) 발견 → 패턴 3분기 보정. L1: files: 가드 즉각 효과. L2: milestone PLAN.md MD032 수동 의무. L3: architecture review가치 (false negative 차단). L4: SKILL.md acceptable omission. 후속 §3-B `v1.88b-precommit-pass-filenames-incremental` + `v1.88c-markdownlint-autofix`.
