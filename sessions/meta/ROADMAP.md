...
마지막 audit: 2026-05-07 (post-v1.88_precommit-performance milestone)
...
(현재 활성 후보 0건 — v1.88 완료 후 신규 발의 trigger 대기)
...
### 3-B. 회귀/장애 evidence 의존 (17건)

| 후속 세션 | Trigger 조건 | 출처 |
|---------|------------|------|
| `v1.88b-precommit-pass-filenames-incremental` | `pass_filenames: true` + smoke 파일-레벨 증분 검사 (608건 → N건). v1.88 L1 효과 측정 후 markdown commit 가속 필요 evidence 시 진입 | `v1.88 REPORT` |
| `v1.88c-markdownlint-autofix` | `markdownlint --fix` pre-commit 통합. v1.88에서 PLAN.md MD032 수동 수정 1 case → evidence 1. 3+ 누적 시 진입 | `v1.88 REPORT` |
| `v1.88d-files-pattern-enumerate-parity` | smoke 본체 enumerate logic ↔ pre-commit `files:` 패턴 1:1 대조 sub-check 신설. v1.88 PR #8 코덱스 P1+P2 false negative 2건 evidence → 향후 신규 hook 등록 시 spec-drift review 단계에 자동 정합 검증 추가 | `v1.88 REPORT L5` |
...
| `v1.81b-roadmap-count-auto-fix` | §3 sub-section count 라벨 drift 재발 evidence 3+. 현재 2건 (v1.81: §3-E `(6건)`→`(5건)` 정정 + v1.85: §3-B `smoke-cross-ref-false-positive-fix` 자동 반영 누락). `harness-roadmap-update` SKILL 5-step → 6-step 확장 (count 라벨 + §8 후속 §3 자동 분류) | `v1.81 REPORT`, `v1.85 REPORT` |
...
| 완료 세션 | 진행 일자 | 산출 |
|---------|---------|------|
| **v1.88_precommit-performance** (milestone) | 2026-05-07 | pre-commit 4 local hook `always_run: true` 제거 + 의미론적 `files:` 패턴 추가 (PR #8 코덱스 봇 P1+P2 review 반영 최종 — smoke-spec-verification: `sessions/.*\.md$\|milestones/.*\.md$` / smoke-scope-contract: `sessions/.*\.md$\|milestones/.*\.md$\|bootstrap/docs/OWNERSHIP\.md$\|claude/commands/harness-meta\.md$` / smoke-cross-ref: `\.(md\|sh\|ps1\|toml\|json\|yaml\|yml\|py\|txt)$` / smoke-claude-md-drift: `CLAUDE\.md$\|tests/smoke-.*\.sh$`) + `tests/CLAUDE.md` 현황표 갱신 (v1.80→v1.88). 효과 실측: phase-2 commit (`tests/CLAUDE.md` 단일 staged) 시 spec-verification + scope-contract 모두 "no files to check Skipped" 출력 — 비관련 hook 자동 skip 검증. 5 pre-commit cycle 40분 wall-clock evidence (v1.87 milestone) 해소 목표 — shell/py/yaml 커밋 시 4 smoke 완전 skip. context7 `pre-commit` `always_run`/`files`/`pass_filenames` 필드 의미론 v1.x 일관 drift=no. 회귀 0 (spec-verification 608/SKIP=4 + scope-contract 198/198 + cross-ref 1/1 + claude-md-drift 16/16 + roadmap-sync 31/0). architecture review (3 관점) — 초안 `smoke-scope-contract: sessions/.*\.md$` false negative 발견 → 3분기 보정. PR #8 후 코덱스 봇 추가 발견 — P1: cross-ref `\.md$`만으로 비-md 참조 대상 deletion/rename 시 hook skip → AT_IMPORT regex 9 확장자로 확장. P2: scope-contract 패턴에 `milestones/` 미포함 → milestone-only PLAN edit 우회 → 4분기로 확장. L1: files: 가드 즉각 효과. L2: milestone PLAN.md MD032 수동 의무. L3: architecture review 가치. L4: SKILL.md acceptable omission. L5: PR review 2차 반영 (smoke 본체 enumerate logic ↔ files: 패턴 1:1 대조 부족 — 향후 spec-drift sub-check 신설 검토). 후속 §3-B `v1.88b-precommit-pass-filenames-incremental` + `v1.88c-markdownlint-autofix` + `v1.88d-files-pattern-enumerate-parity` (L5 evidence 시). 상세 → `milestones/v1.88_precommit-performance/REPORT.md`. |
...
- **v1.88_precommit-performance** (2026-05-07) — pre-commit 4 local hook `always_run: true` 제거 + 의미론적 `files:` 패턴 추가 (PR #8 코덱스 봇 P1+P2 review 반영 최종). 변경 2 파일 (`.pre-commit-config.yaml` + `tests/CLAUDE.md`) + milestone docs 4종. 효과 실측: phase-2 commit (`tests/CLAUDE.md` 단일 staged) 시 spec-verification + scope-contract 모두 "no files to check Skipped" 검증. 5 pre-commit cycle 40분 wall-clock evidence (v1.87 milestone) 해소 목표. context7 `pre-commit` `files`/`always_run`/`pass_filenames` 의미론 v1.x 일관 drift=no. 회귀 0. architecture review 3 관점 — 초안 `smoke-scope-contract: sessions/.*\.md$` false negative 발견 → 3분기 보정. PR #8 후 코덱스 봇 추가 발견 — P1: cross-ref `\.md$` 만으로 비-md 참조 대상 deletion/rename 시 hook skip → AT_IMPORT regex 9 확장자로 확장. P2: scope-contract `milestones/` 미포함 → milestone-only PLAN edit 우회 → 4분기로 확장. L1~L5: files: 가드 즉각 효과 / MD032 수동 의무 / architecture review 가치 / SKILL.md acceptable omission / smoke 본체 ↔ files: 패턴 1:1 대조 부족. 후속 §3-B `v1.88b-precommit-pass-filenames-incremental` + `v1.88c-markdownlint-autofix` + `v1.88d-files-pattern-enumerate-parity`.
