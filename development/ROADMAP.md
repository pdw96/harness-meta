...
{
  "project": "meta",
  "updated": "2026-05-27-v8.10-candidate-registered",
  ...
  "next_candidates": [
    {
      ...
      "target_version": "v8.10",
      "description": "v8.9 oos_2 + L2 origin (사용자 명시 결정으로 등재, 2026-05-27). v8.9 영역 4 wiring 의 실효(audit 가 실제 secret-scan gap detect → 영역 4 매핑 → 자산 참조 draft 분기 실행)는 정적 smoke 범위 밖이라 v8.9 VERIFY 미입증. v8.4→v8.5 패턴(narrative 시뮬레이션→실 재audit 검증)처럼 실 heterogeneous 프로젝트(price-compare working tree)에서 audit-orchestrator 실행으로 영역 4 분기 실작동 + secret-scan over-recommend false-positive 0 검증. 외부 적용 trace = 가벼운 흐름(§7.4, v8.3/v8.5/v8.7 선례). verification-philosophy(v8.6) 제품 역량 검증 vector 정합."
    },
    {
