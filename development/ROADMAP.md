...
{
  "project": "meta",
  "updated": "2026-05-27-v8.10-completed",
  ...
  "milestones": [
    {
      ...
      "status": "completed",
      "trigger": "A_user",
      "milestones_path": "milestones/v8.10/LIGHTWEIGHT.md",
      "summary": "가벼운 흐름 (4 섹션) — v8.9 oos_2 소비. v8.9 영역 4 권고 wiring 의 실효(정적 smoke 범위 밖, agent prompt-time 분기)를 실 heterogeneous 프로젝트(price-compare working tree)에서 검증. OPEN 전 조사로 세션 동결 문제 발견(agent 정의 세션 시작 동결, live spawn probe 가 'ABSENT' 반환) → 사용자 `/reload-plugins` 실행으로 17 agents 재로드 + probe 재확인(영역 4 wiring quote)으로 v8.9 반영 확정 후 live audit. 결과: (A) 영역 4 분기 실작동 확인(gap-analyzer 가 § 4 Read + canonical_asset_gap_judgment 산출) + (B) over-recommend false-positive 0(price-compare secret-guard.py 보유 → gap=false) + (C) detect 결함 검출 = d_2 name-token 매칭이 secret-guard.py(write-time)와 session-start-secret-scan.sh(settings-scan) 직교를 구분 못 해 잠재 false-negative(agent 투명 보고 + 사용자 게이트 이관). 후속 = area4-detect-criterion-refinement(큰 건, wiring 결함 실 수정). /reload-plugins 가 file-based agent 재로드함도 실측(docs 보수 판정과 달리). verification-philosophy 제품 역량 검증 vector 실증 + self-loop 못 본 detect 결함 외부 검출."
    },
    {
      "version": "v8.9",
  ...
  "next_candidates": [
    {
      ...
      "trigger": "B_regression",
      "origin_milestone": "v8.10",
      "target_version": "v8.11",
      "description": "v8.10 (C) 결함 origin (사용자 명시 결정으로 등재, 2026-05-27). v8.10 실 price-compare audit 에서 검출 — harness-gap-analyzer d_2 의 name-token detect 기준이 `secret-guard.py`(PreToolUse Edit|Write, write-time 차단)와 `session-start-secret-scan.sh`(SessionStart, settings*.json 저장 secret warn-only)의 책임 직교를 구분 못 해, 'secret' 토큰 매칭만으로 gap=false 처리 → 잠재 false-negative(실 settings allow-list 격차 남는데 권고 억제). detect 기준을 hook matcher/책임 기반(또는 § 4 표 '권고 case' 컬럼 구조화 조건)으로 정련 = 컨설팅 자산(agent) 변경 = 큰 건(9-stage, v8.4 패턴 — 외부 적용 결함 검출→큰 건 승격). SCOPE_OUT_NOTES architecture(detect 기준 일반화) + hook-asset-library-canonicalization 과 테마 정합."
    },
    {
