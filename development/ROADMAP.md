  ...
  "milestones": [
    {
      ...
      "summary": "사용자 명시 큰 건 결정 (2026-05-28, 토큰 우려 → 사전적 정의 부합 검증 → 분리 확정). 구 ARCHITECTURE.md 안 § 11 개 중 사전적 정의 ('전체의 뼈대·구성요소·관계') 정합 = 구 § 1/2/9/10 (구조) + 구 § 3 (도메인 정체성, 정당한 architecture 일부), 그 외 구 § 4/7.3/7.4/11 = 워크플로우 본질 / 구 § 5/6/7.1/7.2 = 운영 매뉴얼 본질로 혼재. N=2 실용 분리 결정 (ARCHITECTURE 잔류 + WORKFLOW.md + OPERATIONS.md 신설). § 7 sub-split (구 § 7.3/7.4 → WORKFLOW / 구 § 7.1/7.2 → OPERATIONS). 활성 cascade host ≈ 15~20 (큰 건 1 milestone scope). 선행 구조 변경 본질 — codex-adapter-surface-enhancement / design-time-intent-amend 등 다른 v9.1 후보의 § 참조 위치가 본 분리에 의해 변경되므로 v9.1 우선 처리 (사용자 명시 결정)."
    },
    {
      "version": "v9.0",
...
본 schema redesign 정전화 = [`milestones/v5.21/`](milestones/v5.21/) (RESEARCH + DESIGN + REPORT). [`WORKFLOW.md`](WORKFLOW.md) § 1 끝 #3 narrative 본질 변경 (drift 수용 → drift 해소 사례 정전화).
