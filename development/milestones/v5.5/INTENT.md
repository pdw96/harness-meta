...
v5.0_plugin-pivot PROPOSE.next_candidates#4 carry-over. environment-auditor.md Stage B (B1~B6) 는 v4.x install 전제 — v5.0+ Plugin install 환경에서 실행 시 B1~B6 모두 false-negative (6건 전부 FAIL/WARN). 표지만 추가하면 false-negative는 여전히 발생. Stage B 를 install type 감지 기반으로 재구성해야 v5.0+ 사용자도 실질적 audit 수혜를 받는다.
