...
v5.0 PROPOSE#5 + v5.1 PROPOSE#2 carry-over. 현 onboarding 은 git clone → marketplace add ~/harness-meta → plugin install 3 step 으로 clone 이 진입 게이트. GitHub source 지원 시 marketplace add pdw96/harness-meta → plugin install 2 step 으로 단축 — 오픈소스 방문자 첫 접촉 마찰 해소.
