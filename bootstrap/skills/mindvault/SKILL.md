...
> **Local divergence (harness-meta v1.20)**: multi-line YAML frontmatter + markdown reformat. 의미 변경 없음. `disable-model-invocation: true` 추가 (PyPI 설치 + git post-commit hook side effect 차단).
> **Alternative**: [graphify](https://graphify.net/) (active).

# /mindvault

Turn any folder into a searchable knowledge base with a graph, wiki, and BM25 index. Three layers: Search (zero tokens) -> Graph (relationships) -> Wiki (context).
...
If no path was given, use `.` (current directory). Do not ask the user for a path. Follow these steps in order. Do not skip steps.
...
If the import succeeds, print nothing and move to Step 2. **In every subsequent bash block, replace `python3` with `$(cat mindvault-out/.mindvault_python)` to use the correct interpreter.**
...
Replace `QUESTION` with the user's question. Replace `MODE` with `bfs` (default) or `dfs` (if `--dfs` flag used). Present the wiki context to the user as the answer. If tokens_used is very low and wiki_context is empty, suggest running the full pipeline first.
