...
```json
{
  ...
  "decisions": [
    {
      ...
      "alternatives_rejected": ["옵션 X (모든 위치 변경 부재) — cascade 일관성 약화", "옵션 Y (모든 일반 표현 inline 정정) — 토큰 폭증"]
    },
    {
      "id": "D2",
      ...
      "alternatives_rejected": ["옵션 A (이전 D2 'bundled skill 별칭') — spec drift cascade", "옵션 B (단순 'Skill tool invokable built-in command') — bundled skill 범주 부재 명시 누락"]
    },
    {
      "id": "D3",
      ...
      "alternatives_rejected": ["옵션 B (2 phase: 7 cascade target / 2 drift origin) — 의미 단위 같음, 분할 본질 부재"]
    },
    {
      "id": "D4",
      ...
      "alternatives_rejected": ["lightweight 모드 (5 관점 생략) — 사용자 명시 결정 거부, decisive issue 발견 부재 risk"]
    },
    {
      "id": "D5",
      ...
      "alternatives_rejected": ["옵션 (a) phase-1 commit 안 포함 — 산출물 영구 보존 약함", "옵션 (c) 별도 chore commit 추가 — 3 commit 토큰 비용 증가"]
    },
    {
      "id": "D6",
      ...
      "alternatives_rejected": ["옵션 (a) HTML <aside> — Markdown 일관성 약화", "옵션 (b) 인라인 괄호 표현 — 표 안 행 폭 증가"]
    },
    {
      "id": "D7",
      ...
      "alternatives_rejected": ["옵션 (a) v1.17 안 '/review built-in' inline 정정 — spec 정합 표현 변경 부재 자연 (drift 부재)"]
    },
    {
      "id": "D8",
      ...
      "alternatives_rejected": ["옵션 (a) inline overwrite — audit trail 변형 risk, v5.11 L1 패턴 위배"]
    },
    {
      "id": "D9",
  ...
  "phases": [
    {
      ...
      "risks": ["R2 (smoke 회귀 risk, 14 hook 검증 mitigation)", "R3 (footnote 위치 모호 — D6 채택 mitigation)", "R5 (v5.10 audit trail 변형 — D8 채택 [v5.12 정정] footnote 추가 mitigation)", "R6 (v1.17 변경 부재 자연 — D7 footnote 추가만)"]
    }
  ],
  "risk_mitigation": [
  ...
  "five_perspective_review_result": {
