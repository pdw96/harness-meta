...
```json
{
  ...
  "review_summary": {
    "iteration_rounds": [
      {
        "round": 1,
        "trigger": "cycle 7 trigger condition 부분 충족 의문 (upbit commit 부재)",
        "issues": ["upbit commit 부재 = v5.19 L1 evidence isolation 불충족", "stability 3 cycle 연속 evidence는 동일 baseline에서 의미 있는가"],
        "resolution": "cycle 7 강행 + stability 정전화 scope 축소 (Recommended)"
      },
      {
        "round": 2,
        "trigger": "4 관점 검토 decisive 3건 + scope 재정의 의문",
        "issues": ["D11 prefix 정합 — agent .md narrative vs 실 환경 Plugin namespace mismatch", "§ 4 끝 매트릭스화 즉시 vs deferred", "cycle 7 자체 재고 — upbit commit 부재 종합"],
        "resolution": "scenario B 채택 (3-phase bundling) — cycle 7 강행 + § 4 매트릭스화 즉시 + agent .md namespace cascade"
      }
    ],
    "review_perspectives": [
      {
        ...
        "decisive": 0,
        "recommendations": "P1 2 (매핑 표 + D9 rationale) + P2 2 (paragraph 누적 + verify_grep_keyword) + P3 1 (Edit 순서) — 모두 DESIGN narrative 흡수 완료"
      },
      {
        ...
        "verdict": "pass_with_comments",
        "decisive": 1,
        "recommendations": "D1 prefix 정합성 — D11 결정 (prefix 유지 + v5.19 실 evidence 인용) + D14 cascade (scenario B 후 추가 흡수) 로 해소"
      },
      {
        "perspective": "회귀 risk",
        "agent_type": "Explore",
        "verdict": "pass_with_comments",
        "decisive": "1 (조건부)",
        "recommendations": "R4 매트릭스화 trigger 조건부 — D12 grep -c 검증 + scenario B 후 매트릭스화 in-scope 전환으로 해소"
      },
      {
        ...
        "verdict": "pass_with_comments",
        "decisive": 1,
        "recommendations": "P1 sc_* 매핑 명시 — DESIGN narrative 안 sc ↔ phase 매핑 표 흡수 (15/15 매핑 완전성)"
      }
    ],
    "decisive_residual": 0,
    "blocking_issues": 0
