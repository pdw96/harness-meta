...
**Install 정책 본질 + Claude Code Plugin spec 대안** (v4.3_subagent-discovery-path-research 도입): 현 harness-meta 의 install (~/.claude/{commands,hooks,statusline,skills,agents}/ 안 SymbolicLink default + Copy fallback 매핑) 의 본질 근거 = Claude Code spec 안 subagent/command/hook/statusline/skill discovery 경로 `~/.claude/<category>/` 단일 강제 (sub-agents docs / settings docs context7 검증, [code.claude.com/docs/en/sub-agents](https://code.claude.com/docs/en/sub-agents)). 즉 source-of-truth (repo, git tracked, 임의 clone 위치) 와 deployment (~/.claude/, OS-level, spec 의무 위치) 사이 위치 매핑이 install 의 정당성. 다만 Claude Code Plugin spec ([plugins-reference](https://code.claude.com/docs/en/plugins-reference) + [plugin-marketplaces](https://code.claude.com/docs/en/plugin-marketplaces) context7 검증) 안 plugin marketplace local source 지원 (`claude plugin marketplace add ./harness-meta`) + plugin 안 `agents/` + `commands/` + `hooks/` + `statusline/` + `skills/` 자동 인식 + paths 명시 (plugin.json) 으로 default 디렉토리 대체 가능 = install (SymbolicLink/Copy 매핑) 회피 경로 발견. trade-off — 현 install (자연어 'harness-meta 설치해줘' + D7 sequence) vs Plugin install (CLI `claude plugin install <name>@<marketplace>` + plugin lifecycle 표준). Plugin 채택 시 (1) Developer Mode 의존 해소 (.md 파일 영역 SymbolicLink 불요) + (2) ecosystem integrator 정체성 강화 + (3) plugin scope user/project/local 선택권 보유 + (4) plugin enable/disable/uninstall 표준 지원, 단점 = breaking change (D7 narrative 폐기) + 사용자 onboarding flow 변경. v5.0_plugin-pivot 안 전면 채택 설계 (사용자 결정 후 진행).
